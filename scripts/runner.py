...
class runner:
   ...
   def __init__(self):
      ...
      self.run_point_pub = rospy.Publisher('/' + selfName + '/run_loc/', Point, queue_size=10)
      self.vel_pub = rospy.Publisher('/' + selfName + '/cmd_vel/', Twist, queue_size=10)
   ...
   def get_odom_self(self):
      odom_data = rospy.wait_for_message('/' + selfName + '/odom/', Odometry)
